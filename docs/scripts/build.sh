...
then
  ...
  openssl aes-256-cbc -K $encrypted_190d1b6a13b6_key -iv $encrypted_190d1b6a13b6_iv -in github_deploy_key.enc -out github_deploy_key -d
  ...
  git clone git@github.com:egeldenhuys/docs-bin.git
  ...
  echo -e "# docs-bin\n\nThese files were generated by Travis CI from [egeldenhuys/docks/docs](https://github.com/egeldenhuys/docks/tree/$TRAVIS_BRANCH/docs)" > README.md
  git add -A
